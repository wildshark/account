...
$student_category_id= $_GET['student-entry-type'];
...
if ($student_entry_type == 1){
    ...
    //post payment break down to fees bill field paid

    if (isset($_GET['matriculation'])){
        $fees_ledger = "INSERT INTO `fees_bill` (`transDate`, `jDate`, `yearID`, `semesterID`, `studentID`, `revenueID`, `paid`) VALUES ('$tranDate', '$date', '$school_session', '$semesterID', '$student_id', '2','$matriculation')";
        $sql_x=$conn->query($fees_ledger);
    }

    if (isset($_GET['accept-fees'])){
        $fees_ledger = "INSERT INTO `fees_bill` (`transDate`, `jDate`, `studentID`,`revenueID`,`paid`,`yearID`,`semesterID`) VALUES
...
if (empty($description)){
    ...
    $description= $stName." ~ ". $admissionNo." paid ". $paid_amount ."ref.".$ref;
}

//post to fees_payment
...
VALUES ('$tranDate','$date','$student_id','$schoolID','$payType','$ref','$fees_amount','$paid_amount','$level','$semesterID','$school_session','$fees_cost','$discount','$other')";

if ($conn->query($data) === TRUE) {
    //echo "New record created successfully";
    if ($payType == 1){

        //insert in General Ledger ~ Cash book
        $gl="INSERT INTO general_legder(tranDate,GL_date,ticketID,bookID,tranCatID,description,refNo,cashDr,tranTypeID,yearID,semesterID,profitlossID,balanceSheetID)
VALUES ('$tranDate','$date','F','1','1','$description','$ref','$paid_amount','1','$school_session','$semesterID','1','4')";
...
VALUES ('$tranDate','$date','F','1','1','$description','$ref','$paid_amount','2','$school_session','$semesterID','1','4')";
        $ledger=$conn->query($gl);
        if ($ledger == TRUE){
